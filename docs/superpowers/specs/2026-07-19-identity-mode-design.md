...
- 主公局覆盖 **4~8 人**
- B 档规则：发身份、胜负、主公 +1 血、杀人奖惩、主公先手、标准身份隐藏显示、主公局三选一（主公 5 选 1，选定后全场可见主公武将）

### 1.2 非目标（本版明确不做）

- 主公技（激将 / 护驾 / 制霸 / 妄尊等）——继续标「暂不实现」
- 2~3 人身份局
- **主公局 + 随机武将**——主公局只走三选一（主公 5 选 1 → 他人 3 选 1）；随机武将仅乱斗可用
...
| 深度 | B 档，不做主公技 |
| 模式入口 | 先选乱斗/主公局；乱斗可选随机或三选一，**主公局仅三选一** |
| 主公血量 | 固定 `maxHp + 1`（4~8 人相同） |
| 奖惩 | 杀反贼摸 3；主公杀死忠臣弃光**手牌+装备**（**判定区保留**） |
| 先手 | 随机座位发身份 → 亮主公 → 从主公座位 `startTurn` |
| 信息 | 主公公开；自己可见自己的；死后翻开；存活隐藏身份不显示 |
| 选将 | 主公局：主公先选 5 选 1，选完后**全场立刻可见主公武将**；其余按座 3 选 1 |
| 主死且无反无内 | **无胜者** |
...
```
加人
  → 必选模式：乱斗 | 主公局（未选则开始按钮禁用，或点了提示先选模式）
  → 选开局方式：
       乱斗：随机武将 | 三选一（与现有相同）
       主公局：仅「三选一 / 开始身份局」（不提供随机武将）
```

拦截：

- `gameMode === 'identity' && players.length < 4` → 提示「主公局至少需要 4 名玩家」，return
- `gameMode === 'identity' && 点了随机武将` → 拒绝（UI 上主公局不展示/禁用随机入口）
- 乱斗仍 `MIN_PLAYERS = 2`
...
| 武将分配 | 乱斗：`startGame('random'\|'pick')`；主公局：仅 `startGame('pick')` / `g.generalMode='pick'` |
...
- **主公选完后，全场立刻可以看到主公的武将**（名字/立绘/技能），不必等 `g.started === true`。实现上：身份局选将阶段渲染主公座位时，对 `role==='zhu' && p.general` 放开立绘/武将名展示（`avatarReady` 对主公的例外，或等价条件）；其他玩家仍遵守「未 started 不公开武将」
- 调试选将：主公阶段仅主公；他人阶段同现有
- 池不够：`5 + 3*(n-1)` 不足时安全退化（可改为主公/他人直接随机分将，但**不是**大厅「随机武将」入口；与现有池不足退化同思路）
- 非主公玩家在选将阶段仍 `g.started === false`，不提前公开其武将立绘
...
| `victim.role === 'zhong'` 且杀手 `role === 'zhu'` 且存活 | 主公弃光**手牌+装备**；**判定区保留不动**；弃装备**触发** `onLoseEquip`（主公仍存活，不同于阵亡弃装）；日志误杀忠臣 |
...
- 主公局可提示「需 4~8 人」  
- n&lt;4 点开始 → 拦截提示  
- **主公局不展示/禁用「随机武将」**，仅「三选一」开局

### 7.5 选将 banner

- 主公：从 5 名武将中选择  
- 他人等待主公；主公选定后座位上**立刻显示主公武将**  
- 之后他人三选一同现有
...
| `startGame` | 发身份、仅三选一（主公 5→他人 3） | 现有 random/pick |
...
1. 数据 + 配比表 + normalize + 大厅选模式（主公局隐藏随机入口）  
2. 开局发身份 + 主公+1 + 主公起手  
3. `checkWin` 身份胜负 + 死亡翻身份  
4. 击杀奖惩（主杀忠：手牌+装备，判定区保留）  
5. 三选一：主公 5 选 1（选完立刻公开主公武将）→ 他人 3 选 1  
6. UI 身份块 + 结束文案 + 日志  
...
| 10 | 主死无反无内 | **无胜者** |
| 11 | 杀反 | 杀手摸 3 |
| 12 | 主杀忠 | 主弃手牌+装备；**判定区牌仍在** |
| 13 | 闪电劈死主 | 无奖惩，只判胜负 |
| 14 | 死亡 | `roleRevealed` + 日志含身份 |
| 15 | canSeeRole | 主公全可见；己可见己；生藏不可见；死后可见 |
| 16 | 主公 5 选 1 后他人 3 选 1 | 池不重叠、主公先；**主公选定后全场立刻可见其武将** |
| 17 | newGame/回大厅 | role/mode 清空 |
| 18 | 主公局点随机武将 | 拒绝/无入口；乱斗随机仍可用 |

---

## 11. 风险

1. `startGame` 的 `random/pick` 与 `gameMode` 命名混淆；主公局误开 random  
2. 奖惩弃装备与 `onLoseEquip`/旋风 pending 顺序  
3. 主杀忠弃装须触发 `onLoseEquip`（非阵亡路径）；勿误清判定区  
4. 选将阶段：非主公不泄露武将；**主公选定后必须立刻公开**（`avatarReady` 例外）  
5. DB 身份可读——文档诚实说明  
...
|---|---|---|
| 1 | 数据模型 + 配比 | 已确认 |
| 2 | 开局/选将 | 已确认；修订：主公局不做随机武将；主公选定后立刻公开武将 |
| 3 | 胜负+奖惩（主死无反无内→无胜者） | 已确认；修订：主杀忠判定区保留 |
| 4 | UI | 已确认 |
| 5 | 兼容+测试+顺序 | 已确认 |
| 审阅修订 | 用户批注三处 | 已写入本文 |
